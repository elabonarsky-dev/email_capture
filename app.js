...
class EmailCaptureApp {
    ...
    bindElements() {
        ...
        this.checkConnectionBtn = document.getElementById('checkConnection');
    ...
    attachEventListeners() {
        ...
        // Control panel buttons
        this.checkConnectionBtn.addEventListener('click', () => this.checkConnection());
            ...
            if (this.isEffectivelyOnline() && !this.state.isMockMode) {
                ...
                if (this.state.isMockMode) {
                    ...
                    await this.showModal({
                        ...
                        message: "You're almost in.\nWe'll finish verifying your email as soon as connection is restored.",
                        icon: '💾',
                        primaryText: 'OK',
                        showCancel: false,
                        type: 'info'
    ...
    async validateEmailWithZeroBounce(email) {
        ...
        try {
            // Call serverless function instead of direct API call
            // This keeps API keys secure in Netlify environment variables
            this.log('Attempting serverless function at: /api/validate-zerobounce');
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || errorData.message || `ZeroBounce API error: ${response.status}`);
            }

            const data = await response.json();
            this.log('✅ ZeroBounce response (via serverless):', data);
    ...
    async checkConnection() {
        ...
        try {
            ...
            const data = await response.json();
            this.log('Connection check result:', data);
            ...
            message += `ZeroBounce API Key:\n`;
            message += `  ${data.environment.ZEROBOUNCE_API_KEY.exists ? '✅' : '❌'} ${data.environment.ZEROBOUNCE_API_KEY.exists ? 'Configured' : 'MISSING'}\n`;
            if (data.environment.ZEROBOUNCE_API_KEY.exists) {
                message += `  Length: ${data.environment.ZEROBOUNCE_API_KEY.length} chars\n`;
            }
            message += '\n';

            message += `Beehiiv API Key:\n`;
            message += `  ${data.environment.BEEHIIV_API_KEY.exists ? '✅' : '❌'} ${data.environment.BEEHIIV_API_KEY.exists ? 'Configured' : 'MISSING'}\n`;
            if (data.environment.BEEHIIV_API_KEY.exists) {
                message += `  Length: ${data.environment.BEEHIIV_API_KEY.length} chars\n`;
            }
            message += '\n';

            message += `Beehiiv Publication ID:\n`;
            message += `  ${data.environment.BEEHIIV_PUBLICATION_ID.exists ? '✅' : '❌'} ${data.environment.BEEHIIV_PUBLICATION_ID.exists ? 'Configured' : 'MISSING'}\n`;
            if (data.environment.BEEHIIV_PUBLICATION_ID.exists) {
                message += `  Value: ${data.environment.BEEHIIV_PUBLICATION_ID.value}\n`;
            }
            message += '\n';

            // ZeroBounce Test
            message += '🔍 ZeroBounce API Test:\n';
            if (data.zerobounce_test.success) {
                message += `  ✅ Connection successful\n`;
                message += `  Credits: ${data.zerobounce_test.credits || 'N/A'}\n`;
            } else {
                message += `  ❌ Connection failed\n`;
                message += `  Error: ${data.zerobounce_test.error || 'Unknown'}\n`;
            }
            message += '\n';

            // Beehiiv Test
            message += '📧 Beehiiv API Test:\n';
            if (data.beehiiv_test.success) {
                message += `  ✅ Connection successful\n`;
                message += `  Status: ${data.beehiiv_test.status}\n`;
                if (data.beehiiv_test.response && data.beehiiv_test.response.name) {
                    message += `  Publication: ${data.beehiiv_test.response.name}\n`;
                }
            } else {
                message += `  ❌ Connection failed\n`;
                message += `  Error: ${data.beehiiv_test.error || 'Unknown'}\n`;
                if (data.beehiiv_test.response) {
                    message += `  Response: ${JSON.stringify(data.beehiiv_test.response, null, 2)}\n`;
                }
            }

            // Overall status
            const allSuccess = data.zerobounce_test.success && data.beehiiv_test.success;
            message += '\n';
            message += allSuccess ? '✅ All systems operational!' : '⚠️ Some systems have issues';

            await this.showAlert(message, '🔌 Connection Test', allSuccess ? '✅' : '⚠️');
            this.showStatus(allSuccess ? '✅ Connection test passed' : '⚠️ Connection test issues', allSuccess ? 'success' : 'warning');

        } catch (error) {
            this.log('❌ Connection check error:', error);
            await this.showError(
                `Failed to check connection:\n\n${error.message}\n\nMake sure:\n1. You're deployed to Netlify\n2. Environment variables are set\n3. Functions are deployed correctly`,
    ...
    async exportToCsv() {
        ...
        try {
            ...
            const BOM = '\uFEFF';
            let csv = BOM + 'Email,Status,Event Tag,Date\n';

            validatedEmails.forEach(item => {
                // Convert timestamp to human-readable local datetime
                const date = new Date(item.timestamp);
                // Format as YYYY-MM-DD HH:MM:SS in local time for Excel compatibility
                ...
                csv += `"${item.email}","${item.status}","${eventTag}","${localDate}"\n`;
            });
